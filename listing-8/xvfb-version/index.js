...
const Xvfb = require('xvfb'); // Include the xvfb virtual framebuffer module.

webServer.start()
    .then(server => {
        //todo: want to refactor this now so that capture chart can be reused!

        const xvfb = new Xvfb();        
        xvfb.startSync(); // Start the virtual framebuffer.
        ...
        return nightmare.goto("http://localhost:3000") // Point the browser at the web server we just started.
            ...
            .then(() => xvfb.stopSync()) // Shutdown the virtual framebuffer.
